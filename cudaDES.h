...
uint64_t cudaFunction(uint64_t key, const uint64_t plain, const uint64_t cipher);
...
//__global__ void DESkernel(volatile int* keyfound, uint64_t* key, const uint64_t plain, const uint64_t cipher, volatile uint32_t *d_state_ptr);
//__device__ int des_setkey_enc_cuda( des_context *ctx, const unsigned char key[DES_KEY_SIZE] );
//__device__ static void des_setkey_cuda( uint32_t SK[32], const uint64_t key /*const unsigned char key[DES_KEY_SIZE] */);
//__device__ int des_crypt_ecb_cuda( des_context *ctx, const unsigned char input[8], unsigned char output[8] );
////__device__ int des_crypt_ecb_cuda64( des_context *ctx, const uint64_t input, uint64_t &output );
